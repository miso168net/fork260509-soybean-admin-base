<!-- BASE-WEB-MODAL-WIRING(g) (014-user-center)：net-new 信箱卡——rev3 025 藍本逐項復刻（信箱 input col1＋驗證碼
  佔位組 col2）；儲存只送 {userEmail} 單欄（部分更新、FR-008）；佔位組純前端、點擊 comingSoon 零網路寫入（FR-012）；
  與 phone-card.vue 刻意同構。憲法 §III.2(g) 授權。★新檔零原行（example 基線無此檔）。 -->
...
interface Props {
  /** 個人中心 canonical model（index.vue 持有、三卡 prop 共綁；本卡編 userEmail） */
...
// 信箱格式前端寬鬆把關（trigger=change、非 required 空值容忍——與管理面同水位、後端不驗格式）
const rules = {
...
async function handleSave() {
  ...
  const { error } = await fetchUpdateProfile({
    userEmail: props.model.userEmail ?? ''
  });
  if (!error) {
