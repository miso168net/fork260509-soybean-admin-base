...
// audit 3 子表共用父路由 manage_audit → 複合 storageKey `<route.name>:login` 避免互覆寫（analyze F3）
const { handleUpdateSorter, getColumnSortProps, sortString, clearAll } = useTableSort({
  storageKey: `${route.name as string}:login`,
  validKeys: ['createTime', 'attemptedUserName', 'success', 'ipConfidence', 'peerIp', 'realIp', 'xForwardedFor', 'region']
});
