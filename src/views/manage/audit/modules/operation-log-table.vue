...
// audit 3 子表共用父路由 manage_audit → 複合 storageKey `<route.name>:operation` 避免互覆寫（analyze F3）
const { handleUpdateSorter, getColumnSortProps, sortString, clearAll } = useTableSort({
  storageKey: `${route.name as string}:operation`,
  validKeys: [
    'createTime',
    'operation',
    'entityTable',
    'entityId',
    'operatorIpConfidence',
    'operatorPeerIp',
    'operatorRealIp',
    'operatorXForwardedFor'
  ]
});
...
const { columns, data, loading, getDataByPage, mobilePagination } = useNaivePaginatedTable({
  ...
  api: () => fetchGetOperationLog({ ...searchParams.value, sort: sortString.value || undefined }),
  ...
  columns: () => [
    ...
    },
    {
      ...
      minWidth: 120,
      // [rev3-inline 023-list-column-sort MW(f)] 可排序欄
      ...getColumnSortProps('entityTable')
    },
    {
      key: 'entityId',
    ...
    },
    {
      ...
      minWidth: 110,
      // [rev3-inline 023-list-column-sort MW(f)] 可排序欄
      ...getColumnSortProps('operatorIpConfidence'),
    ...
    },
    {
      ...
      minWidth: 130,
      // [rev3-inline 023-list-column-sort MW(f)] 可排序欄
      ...getColumnSortProps('operatorPeerIp'),
    ...
    },
    {
      ...
      minWidth: 130,
      // [rev3-inline 023-list-column-sort MW(f)] 可排序欄
      ...getColumnSortProps('operatorRealIp'),
    ...
    },
    {
      ...
      minWidth: 180,
      // [rev3-inline 023-list-column-sort MW(f)] 可排序欄
      ...getColumnSortProps('operatorXForwardedFor'),
...
async function onExport() {
  ...
  const { error, data } = await fetchExportOperationLog({ ...searchParams.value, sort: sortString.value || undefined });
