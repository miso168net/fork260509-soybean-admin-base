...
declare namespace App {
  ...
  namespace I18n {
    ...
    type Schema = {
      ...
      // （映射 wire msg key → 在地化；逐鍵鏡像 locale backend 樹 **76 鍵**＝003-auth-session
      ...
      // biz.policy.* 一鍵＋007-user-password-admin 之 auth.session.kickedByAdmin 一鍵與
      // biz.user.* 二十鍵＋008-audit-settings-pages 之 biz.audit.* 二鍵——各刀的單一權威＝
      // 該刀 contracts/msg-keys.md）。★必填（非 optional）＝en-us.ts／
      ...
      backend: {
        ...
        biz: {
          // 008-audit-settings-pages T015 二鍵（單一權威＝
          // specs/008-audit-settings-pages/contracts/msg-keys.md；purgeBelowFloor 為
          // 攜參鍵、後端 BizData 帶 {minDays}——佔位符字面權威同該表）
          audit: {
            invalidTable: string;
